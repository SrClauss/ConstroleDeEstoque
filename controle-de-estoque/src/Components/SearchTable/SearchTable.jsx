...
export default function SearchTable({ data, onDataChange, stateSelected, nullData }) {
      ...
      <Table>
        ...
        {rows.map((row, index) => (
          <ValueRow
            ...
            cells={row}
            onDoubleClick={(id, response) => handleSelect(id, index, response)}
  ...
  return (
    ...
    >
      {cells.map((cell, index) => (
        <TableCell key={index} sx={{ cursor: "pointer" }}>{cell}</TableCell>
      ))}
    </TableRow>
  );
}
